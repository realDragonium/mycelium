...
def get_substrate_dump() -> dict[str, Any]:
    ...
    Returns:
        ...
        - statements: [{id, text, mentions: [entity_id]}] — entity_ids are
          deduplicated from the underlying name_id mentions, since the UI
          renders mentions as entity chips.
        - links: [{from, to, link_type}].
    """
    ...
    for r in link_rows:
        ...
        if r["when_hash"] != "NONE":
            tree = store._load_when_tree(conn, r["link_id"])
            if tree is not None:
                entry["when"] = tree
        links.append(entry)

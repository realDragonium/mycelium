...
function NamesEditor({entity, entities, onChanged}) {
  ...
  return <section className="names-panel"><h3>Edit names</h3>
    <form onSubmit={event => {event.preventDefault(); run(false);}}><fieldset disabled={busy}>
      <label>Action<select value={kind} onChange={event => changeKind(event.target.value)}>
        <option value="add">Add alias</option><option value="correct">Correct spelling</option><option value="prefer">Change preferred name</option><option value="move">Move alias to another concept</option><option value="split">Split names into a new concept</option><option value="merge">Merge into another concept</option><option value="remove">Remove alias</option>
      </select></label>
      {['correct','prefer','move','remove'].includes(kind) && <label>Name<select required value={nameId} onChange={event => {setNameId(event.target.value); setPreview(null); if (kind === 'correct') setText(authored.find(name => name.id === event.target.value)?.text || '');}}><option value="">Choose a name</option>{authored.map(name => <option key={name.id} value={name.id}>{name.text}</option>)}</select></label>}
      ...
      {['move','merge'].includes(kind) && <label>Destination concept<select required value={target} onChange={event => {setTarget(event.target.value); setPreview(null); setPreferred(''); if (kind === 'merge') setDescription(entities.find(item => item.id === event.target.value)?.description || '');}}><option value="">Choose a concept</option>{entities.filter(item => item.id !== entity.id).map(item => <option key={item.id} value={item.id}>{item.label}</option>)}</select></label>}
      ...
      {['split','merge'].includes(kind) && <>
        <label>Preferred name<select required value={preferred} onChange={change(setPreferred)}><option value="">Choose the displayed name</option>{preferredOptions.map(name => <option key={name.id} value={name.id}>{name.text}</option>)}</select></label>
        ...
        <label>{kind === 'split' ? 'New concept description' : 'Surviving description'}<textarea maxLength={20000} value={description} onChange={change(setDescription)} /></label>
      </>}
...
function NamesConcept({id, entities, canWrite, refresh, onChanged}) {
  ...
  return <div><section className="names-panel"><h2>{entity.label}</h2><p>{entity.description || 'No description.'}</p>
    ...
    <h3>Example statements</h3><NamesExamples examples={detail.examples} count={detail.example_count}/>
  ...
  {canWrite && <NamesEditor key={id + ':' + detail.revision} entity={entity} entities={entities} onChanged={onChanged}/>}
...
function NamesWorkspace() {
  ...
  const changed = id => {if (id) setSelected(id); setRefresh(value => value + 1); setNotice('Change saved. Discovery is being recomputed.'); window.dispatchEvent(new Event('mycelium-data-changed'));};
  ...
  return <main className="names-workspace"><h1>Names &amp; aliases</h1><p>Keep the names for each concept together. Statements keep their original wording.</p>
    ...
    {!catalogue ? <p>Loading names…</p> : tab === 'suggestions' && Suggestions ? <Suggestions canWrite={catalogue.can_write} onChanged={() => changed()}/> : <>
      ...
      {selected ? <NamesConcept id={selected} entities={catalogue.entities} canWrite={catalogue.can_write} refresh={refresh} onChanged={changed}/> : <p>Select a concept to inspect its names and examples.</p>}
      </div>
    </>}
  </main>;
}

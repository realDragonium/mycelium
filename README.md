...
`cross-encoder/nli-deberta-v3-base`), `MYCELIUM_NLI_CONFIDENCE` (default `0.7`),
and the per-batch `MYCELIUM_NLI_MAX_PAIRS` budget (default `400`).
...
| `submit_connected_batch([{kind, text, links?, allow_phrasing_violations?, mention_hints?}], title?)` | `POST /submit-connected-batch` | Submit an already-extracted batch and get back an open **draft** instead of a write. Phrasing rejection, `@N` sibling refs, and cascade behave exactly as in the batch upsert; `links` are outgoing only and an item carrying `incoming_links` is rejected. Surviving statements go through the connection pipeline, which proposes typed links from explicit relational cues in the text, merge candidates, and contradiction flags against what already exists. Nothing is applied: the batch and every proposal land as separate draft ops — each proposal op carries its provenance, the batch op has none — strike with `discard_draft_op`, then `submit_draft`. Optional `mention_hints` widen candidate discovery for the call without writing mentions. Two index spaces in the response: `depends_on` reports original input positions, while `batch_index` and proposal `@N` targets report accepted-item positions after rejections. Returns `{draft_id, results, proposals, links, merges, conflicts, related, dropped_merges, unresolved_hints, nli, draft}`. |
| `ingest_text(text, title?)` | `POST /ingest-text` | Hand over raw or lightly structured prose — a paragraph, a few sentences, a bullet list — and get back the same kind of open **draft** `submit_connected_batch` returns. Segments the text on the phrasing catalog's own atomicity cues (semicolons, compound phrases, conditional openers, conservative verb coordination) instead of rejecting compound input, assigns a `kind` per fragment by phrasing shape, and runs the survivors through the same connection pipeline under the same catalog validation. A conditional opener also proposes a `requires` edge from claim to condition; no other relation is inferred, no text is rewritten, and no kind is guessed. Every fragment it cannot vouch for — still compound, no shape, disagreeing shapes, or shape-then-catalog-rejected — lands in the draft as a non-replaying `flag` op that `apply_draft` skips with `{"skipped": "flag"}`; strike one with `discard_draft_op` after authoring the fragment yourself. Returns `{draft_id, fragments, items, flags, condition_links, results, proposals, links, merges, conflicts, related, dropped_merges, unresolved_hints, nli, draft}`. |

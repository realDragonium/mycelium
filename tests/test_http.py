...
def test_http_full_surface(tmp_path, monkeypatch):
    with _client(tmp_path, monkeypatch, fake_embed_factory()) as client:
        ...
        r = client.post("/upsert-name", json={"text": "bogus", "entity_id": "ent_missing"})
...
def test_search_min_score_filters_low_matches(tmp_path, monkeypatch):
    with _client(tmp_path, monkeypatch, deterministic_embed) as client:
        client.post("/upsert-statement", json={"kind": "event", "text": "alpha unique", "mentions": [], "links": []})
        client.post("/upsert-statement", json={"kind": "event", "text": "beta unique", "mentions": [], "links": []})
...
def test_search_depth_expands_children_and_parents(tmp_path, monkeypatch):
    with _client(tmp_path, monkeypatch, deterministic_embed) as client:
        ...
        a = client.post("/upsert-statement", json={"kind": "event", "text": "node a", "mentions": [], "links": []}).json()["statement_id"]
        b = client.post("/upsert-statement", json={"kind": "event", "text": "node b", "mentions": [], "links": []}).json()["statement_id"]
        c = client.post("/upsert-statement", json={"kind": "event", "text": "node c", "mentions": [], "links": []}).json()["statement_id"]
        client.post(
            "/upsert-statement",
            json={"kind": "event", "text": "node a", "mentions": [], "links": [{"to_id": b, "link_type": "part"}], "id": a},
        )
        client.post(
            "/upsert-statement",
            json={"kind": "event", "text": "node b", "mentions": [], "links": [{"to_id": c, "link_type": "part"}], "id": b},
        )
...
def test_search_mentions_shape(tmp_path, monkeypatch):
    ...
    with _client(tmp_path, monkeypatch, deterministic_embed) as client:
        ...
        client.post(
            "/upsert-statement",
            json={"kind": "event", "text": "the recruiter opens the dashboard", "links": []},
        )
        hits = client.post(
            "/search-statements", json={"query": "the recruiter opens the dashboard", "min_score": 0.99}
        ).json()
        assert len(hits) == 1
...
def test_upsert_name_idempotent_then_conflict(tmp_path, monkeypatch):
    with _client(tmp_path, monkeypatch, deterministic_embed) as client:
        ...
        r = client.post(
            "/upsert-name", json={"text": "sign-in", "entity_id": other_id}
        )
        assert r.status_code == 400
...
def test_merge_entities_preserves_statement_mentions(tmp_path, monkeypatch):
    with _client(tmp_path, monkeypatch, deterministic_embed) as client:
        a = client.post("/upsert-entity", json={"name": "Login", "description": ""}).json()["entity_id"]
        b = client.post("/upsert-entity", json={"name": "Sign-in", "description": ""}).json()["entity_id"]
        # statements mentioning each separately
        client.post("/upsert-statement", json={"kind": "event", "text": "statement alpha", "mentions": ["Login"], "links": []})
        client.post("/upsert-statement", json={"kind": "event", "text": "statement beta", "mentions": ["Sign-in"], "links": []})
        ...
        names = {hits["mentions"][0]["name"]
                 for hits in [
                     client.post("/search-statements", json={"query": q, "min_score": 0.99}).json()[0]
...
def test_add_and_remove_links_bulk(tmp_path, monkeypatch):
    with _client(tmp_path, monkeypatch, deterministic_embed) as client:
        ...
        r = client.post("/add-links", json={"links": [
        ...
        hits = client.post("/search-statements", json={"query": "node a", "min_score": 0.99}).json()
        ...
        r = client.post("/remove-links", json={"links": [
            {"from_id": a, "to_id": c, "link_type": "triggers"},
        ]})
...
def test_upsert_statement_with_incoming_links(tmp_path, monkeypatch):
    with _client(tmp_path, monkeypatch, deterministic_embed) as client:
        ...
        p1 = client.post("/upsert-statement", json={"kind": "event", "text": "parent one", "mentions": [], "links": []}).json()["statement_id"]
        p2 = client.post("/upsert-statement", json={"kind": "event", "text": "parent two", "mentions": [], "links": []}).json()["statement_id"]

        # Create a new child wired to both parents in a single call
        r = client.post("/upsert-statement", json={"kind": "event", 
        ...
        for parent_id in (p1, p2):
            hits = client.post("/search-statements", json={
                "query": "parent",
                "limit": 5,
                "min_score": -1.0,
            }).json()
        ...
        client.post("/upsert-statement", json={"kind": "event", 
        ...
        hits = client.post("/search-statements", json={
            "query": "parent", "limit": 5, "min_score": -1.0,
        }).json()
        p1_hit = next(h for h in hits if h["id"] == p1)
        p2_hit = next(h for h in hits if h["id"] == p2)
        assert any(l["to_id"] == child and l["link_type"] == "part" for l in p1_hit["links"])
        assert any(l["to_id"] == child and l["link_type"] == "triggers" for l in p2_hit["links"])
...
def test_get_statement_returns_both_link_directions(tmp_path, monkeypatch):
    with _client(tmp_path, monkeypatch, deterministic_embed) as client:
        ...
        a = client.post("/upsert-statement", json={"kind": "event", "text": "the dashboard node", "links": []}).json()["statement_id"]
        b = client.post("/upsert-statement", json={"kind": "event", "text": "node b", "links": [{"to_id": a, "link_type": "part"}]}).json()["statement_id"]
...
def test_entity_links_lifecycle(tmp_path, monkeypatch):
    ...
    with _client(tmp_path, monkeypatch, deterministic_embed) as client:
        ...
        parent = client.post("/upsert-entity", json={"name": "MegaCorp", "description": ""}).json()["entity_id"]
        sub_a = client.post("/upsert-entity", json={"name": "MegaCorp Cloud", "description": ""}).json()["entity_id"]
        sub_b = client.post("/upsert-entity", json={"name": "MegaCorp Logistics", "description": ""}).json()["entity_id"]
        ...
        r = client.post("/add-entity-links", json={"links": [
            {"from_entity_id": parent, "to_entity_id": sub_a, "link_type": "contains"},
            {"from_entity_id": parent, "to_entity_id": sub_b, "link_type": "contains"},
        ]}).json()
        assert r == {"inserted": 2}
        ...
        assert sorted(l["to_entity_id"] for l in parent_body["links"]) == sorted([sub_a, sub_b])
        sub_a_body = client.post("/get-entity", json={"id": sub_a}).json()
        assert sub_a_body["incoming_links"] == [{"from_entity_id": parent, "link_type": "contains"}]
        ...
        # Self-loop rejected
        r = client.post("/add-entity-links", json={"links": [
            {"from_entity_id": parent, "to_entity_id": parent, "link_type": "contains"},
        ]})
        assert r.status_code == 400

        # Unknown entity rejected before any insert
        r = client.post("/add-entity-links", json={"links": [
            {"from_entity_id": parent, "to_entity_id": "ent_missing", "link_type": "contains"},
        ]})
        assert r.status_code == 400

        # Remove one edge, leave the other in place
        r = client.post("/remove-entity-links", json={"links": [
            {"from_entity_id": parent, "to_entity_id": sub_a, "link_type": "contains"},
        ]}).json()
        assert r == {"removed": 1}
        parent_body = client.post("/get-entity", json={"id": parent}).json()
...
def test_merge_entities_rewrites_entity_links(tmp_path, monkeypatch):
    ...
    with _client(tmp_path, monkeypatch, deterministic_embed) as client:
        parent = client.post("/upsert-entity", json={"name": "Parent", "description": ""}).json()["entity_id"]
        a = client.post("/upsert-entity", json={"name": "ChildA", "description": ""}).json()["entity_id"]
        b = client.post("/upsert-entity", json={"name": "ChildB", "description": ""}).json()["entity_id"]
        sibling = client.post("/upsert-entity", json={"name": "Sibling", "description": ""}).json()["entity_id"]

        # Wire: Parent contains A, Parent contains B, Sibling partner-of A
        client.post("/add-entity-links", json={"links": [
            {"from_entity_id": parent, "to_entity_id": a, "link_type": "contains"},
            {"from_entity_id": parent, "to_entity_id": b, "link_type": "contains"},
            {"from_entity_id": sibling, "to_entity_id": a, "link_type": "partner-of"},
        ]})
        ...
        r = client.post("/merge-entities", json={"from_entity_id": a, "into_entity_id": b})
        assert r.status_code == 200
        ...
        incoming_pairs = sorted((l["from_entity_id"], l["link_type"]) for l in b_body["incoming_links"])
...
def test_get_entity_returns_all_names(tmp_path, monkeypatch):
    with _client(tmp_path, monkeypatch, deterministic_embed) as client:
        eid = client.post("/upsert-entity", json={"name": "Login", "description": "auth surface"}).json()["entity_id"]
        ...
        assert names == [
            "Login", "Logins", "log-in", "log-ins", "sign-in", "sign-ins",
        ]
...
def test_list_entities_pagination_and_prefix(tmp_path, monkeypatch):
    with _client(tmp_path, monkeypatch, deterministic_embed) as client:
        ...
        body = client.post("/list-entities", json={"prefix": "", "limit": 50, "offset": 0}).json()
        assert body["total"] == 4
        assert [e["name"] for e in body["entities"]] == ["Apple", "Apricot", "Banana", "Coconut"]

        body = client.post("/list-entities", json={"prefix": "Ap", "limit": 50, "offset": 0}).json()
        assert [e["name"] for e in body["entities"]] == ["Apple", "Apricot"]

        body = client.post("/list-entities", json={"prefix": "", "limit": 2, "offset": 1}).json()
...
def test_list_statements_pagination(tmp_path, monkeypatch):
    with _client(tmp_path, monkeypatch, deterministic_embed) as client:
        ids = []
        for i in range(5):
            ids.append(client.post("/upsert-statement", json={"kind": "event", "text": f"statement {i}", "mentions": [], "links": []}).json()["statement_id"])
...
def test_list_statements_filter_by_entity_and_name(tmp_path, monkeypatch):
    with _client(tmp_path, monkeypatch, deterministic_embed) as client:
        ...
        dashboard_eid = client.post("/upsert-entity", json={"name": "dashboard", "description": ""}).json()["entity_id"]
        client.post("/upsert-entity", json={"name": "invoice", "description": ""})
        client.post("/upsert-name", json={"text": "workspace", "entity_id": dashboard_eid})

        # Three statements mention dashboard (one via the "workspace" alias),
        # one mentions only invoice, one mentions nothing.
        b1 = client.post("/upsert-statement", json={"kind": "event", "text": "the dashboard loads", "links": []}).json()["statement_id"]
        b2 = client.post("/upsert-statement", json={"kind": "event", "text": "the workspace opens via sso", "links": []}).json()["statement_id"]
        b3 = client.post("/upsert-statement", json={"kind": "event", "text": "the dashboard shows the invoice", "links": []}).json()["statement_id"]
        b4 = client.post("/upsert-statement", json={"kind": "event", "text": "the invoice is paid", "links": []}).json()["statement_id"]
        client.post("/upsert-statement", json={"kind": "event", "text": "unrelated", "links": []})

        # Filter by entity_id — dashboard matches b1, b2 (via alias), b3
        body = client.post("/list-statements", json={"limit": 50, "offset": 0, "entity_id": dashboard_eid}).json()
        assert body["total"] == 3
        assert {b["id"] for b in body["statements"]} == {b1, b2, b3}

        # Filter by name "dashboard" — same set
        body = client.post("/list-statements", json={"limit": 50, "offset": 0, "name": "dashboard"}).json()
        assert {b["id"] for b in body["statements"]} == {b1, b2, b3}

        # Filter by alias "workspace" — collapses to the same entity, same set
        body = client.post("/list-statements", json={"limit": 50, "offset": 0, "name": "workspace"}).json()
        assert {b["id"] for b in body["statements"]} == {b1, b2, b3}

        # Pagination respects the filter
        body = client.post("/list-statements", json={"limit": 2, "offset": 0, "name": "dashboard"}).json()
        assert body["total"] == 3
        assert len(body["statements"]) == 2

        # b3 mentions both dashboard and invoice — DISTINCT shouldn't double-count
        body = client.post("/list-statements", json={"limit": 50, "offset": 0, "name": "invoice"}).json()
        assert body["total"] == 2
        assert {b["id"] for b in body["statements"]} == {b3, b4}

        # Validation: both filters together → 400
        r = client.post("/list-statements", json={"limit": 50, "offset": 0, "entity_id": dashboard_eid, "name": "dashboard"})
        assert r.status_code == 400

        # Unknown name → 400
        r = client.post("/list-statements", json={"limit": 50, "offset": 0, "name": "NonExistent"})
        assert r.status_code == 400

        # Unknown entity_id → 400
        r = client.post("/list-statements", json={"limit": 50, "offset": 0, "entity_id": "ent_does_not_exist"})
        assert r.status_code == 400
...
def test_kind_filter_on_list_search_grep(tmp_path, monkeypatch):
    ...
    with _client(tmp_path, monkeypatch, deterministic_embed) as client:
        ev = client.post("/upsert-statement", json={
            "kind": "event", "text": "user logs in",
            "mentions": [], "links": [],
        }).json()["statement_id"]
        st = client.post("/upsert-statement", json={
            "kind": "state", "text": "session is active",
            "mentions": [], "links": [],
        }).json()["statement_id"]
        cap = client.post("/upsert-statement", json={
            "kind": "capability", "text": "admin can revoke session",
            "mentions": [], "links": [], "allow_phrasing_violations": True,
        }).json()["statement_id"]
        ...
        body = client.post("/grep-statements", json={
            "query": "session", "kind": "state",
        }).json()
        assert {b["id"] for b in body["statements"]} == {st}

        body = client.post("/grep-statements", json={
            "query": "session", "kind": "capability",
        }).json()
        assert {b["id"] for b in body["statements"]} == {cap}

        # search filters by kind too
        hits = client.post("/search-statements", json={
            "query": "session", "kind": "state", "limit": 10, "min_score": -1.0,
        }).json()
...
def test_get_statement_returns_kind(tmp_path, monkeypatch):
    with _client(tmp_path, monkeypatch, deterministic_embed) as client:
        sid = client.post("/upsert-statement", json={
            "kind": "state", "text": "user is signed in",
            "mentions": [], "links": [],
        }).json()["statement_id"]
        body = client.post("/get-statements", json={"ids": [sid]}).json()["statements"][0]
...
def test_search_mentions_filter(tmp_path, monkeypatch):
    with _client(tmp_path, monkeypatch, deterministic_embed) as client:
        ...
        b_both = client.post("/upsert-statement", json={"kind": "event", "text": "the dashboard shows the invoice", "links": []}).json()["statement_id"]
        b_dashboard_only = client.post("/upsert-statement", json={"kind": "event", "text": "the dashboard opens via sso", "links": []}).json()["statement_id"]
        b_neither = client.post("/upsert-statement", json={"kind": "event", "text": "unrelated topic", "links": []}).json()["statement_id"]

        # Filter on a single entity — both dashboard statements match
        hits = client.post("/search-statements", json={"query": "dashboard", "limit": 10, "min_score": -1.0, "mentions": ["dashboard"]}).json()
        assert {h["id"] for h in hits} == {b_both, b_dashboard_only}

        # AND semantics — must mention both dashboard AND invoice
        hits = client.post("/search-statements", json={"query": "dashboard", "limit": 10, "min_score": -1.0, "mentions": ["dashboard", "invoice"]}).json()
        assert [h["id"] for h in hits] == [b_both]

        # Unknown name in filter → 400
        r = client.post("/search-statements", json={"query": "x", "mentions": ["NonExistent"]})
        assert r.status_code == 400
...
def test_unknown_words_mention_nothing(tmp_path, monkeypatch):
    ...
    with _client(tmp_path, monkeypatch, deterministic_embed) as client:
        known_id = client.post("/upsert-entity", json={"name": "dashboard", "description": ""}).json()["entity_id"]

        # Text mentions the known name → one derived mention.
        sid = client.post("/upsert-statement", json={"kind": "event",
            "text": "the dashboard refreshes", "links": [],
        }).json()["statement_id"]
        body = client.post("/get-statements", json={"ids": [sid]}).json()["statements"][0]
        assert {m["entity_id"] for m in body["mentions"]} == {known_id}

        # Text whose distinctive words match no existing entity → no mentions.
        other = client.post("/upsert-statement", json={"kind": "event",
            "text": "the analytics warehouse synchronizes nightly", "links": [],
        }).json()["statement_id"]
        body = client.post("/get-statements", json={"ids": [other]}).json()["statements"][0]
        assert body["mentions"] == []

        # And no entity was auto-created for any of those unknown words.
        ents = client.post("/list-entities", json={"prefix": "", "limit": 50, "offset": 0}).json()
...
def test_upsert_after_merge_reuses_deleted_vector_slot(tmp_path, monkeypatch):
    ...
    with _client(tmp_path, monkeypatch, deterministic_embed) as client:
        ...
        a = client.post("/upsert-statement", json={"kind": "event", "text": "statement a", "mentions": [], "links": []}).json()["statement_id"]
        b = client.post("/upsert-statement", json={"kind": "event", "text": "statement b", "mentions": [], "links": []}).json()["statement_id"]
        ...
        # left orphaned.
        r = client.post("/upsert-statement", json={"kind": "event", "text": "statement c", "mentions": [], "links": []})
        ...
        hits = client.post("/search-statements", json={"query": "statement c", "limit": 5, "min_score": -1.0}).json()
...
def test_merge_statements_unions_links_and_drops_self_loops(tmp_path, monkeypatch):
    with _client(tmp_path, monkeypatch, deterministic_embed) as client:
        ...
        parent_of_a = client.post("/upsert-statement", json={"kind": "event", "text": "parent of a", "links": []}).json()["statement_id"]
        parent_of_b = client.post("/upsert-statement", json={"kind": "event", "text": "parent of b", "links": []}).json()["statement_id"]
        shared_child = client.post("/upsert-statement", json={"kind": "event", "text": "shared child", "links": []}).json()["statement_id"]
        a_only_child = client.post("/upsert-statement", json={"kind": "event", "text": "a only child", "links": []}).json()["statement_id"]
        ...
        a = client.post("/upsert-statement", json={"kind": "event",
        ...
        b = client.post("/upsert-statement", json={"kind": "event",
        ...
        # Merge A into B
        r = client.post("/merge-statements", json={
            "from_id": a, "into_id": b,
        })
        assert r.status_code == 200
        body = r.json()
        ...
        incoming = sorted((l["from_id"], l["link_type"]) for l in body["incoming_links"])
        assert incoming == sorted([(parent_of_a, "part"), (parent_of_b, "part")])

        # Search no longer surfaces A
        hits = client.post("/search-statements", json={"query": "statement a about the invoice", "min_score": 0.5, "limit": 10}).json()
...
def test_upsert_statement_warns_on_near_duplicate(tmp_path, monkeypatch):
    with _client(tmp_path, monkeypatch, deterministic_embed) as client:
        # Existing statement
        first = client.post("/upsert-statement", json={"kind": "event", "text": "user signs in", "mentions": [], "links": []}).json()
        assert first["near_duplicates"] == []  # nothing else in the substrate yet

        # Same text again → near-duplicate of the first
        second = client.post("/upsert-statement", json={"kind": "event", "text": "user signs in", "mentions": [], "links": []}).json()
        ...
        third = client.post("/upsert-statement", json={"kind": "event", "text": "completely different topic", "mentions": [], "links": []}).json()
        assert third["near_duplicates"] == []

        # Update path: id provided, self should be excluded from results
        fourth = client.post("/upsert-statement", json={"kind": "event", 
            "text": "user signs in",
            "mentions": [], "links": [],
            "id": first["statement_id"],
        }).json()
        # The OTHER statement with same text is still a near-dup; first itself is excluded.
        assert all(nd["id"] != first["statement_id"] for nd in fourth["near_duplicates"])
        assert any(nd["id"] == second["statement_id"] for nd in fourth["near_duplicates"])


def test_upsert_statements_batch_warns_on_internal_duplicates(tmp_path, monkeypatch):
    with _client(tmp_path, monkeypatch, deterministic_embed) as client:
        r = client.post("/upsert-statements", json={
            "statements": [
                {"kind": "event", "text": "alpha unique", "mentions": [], "links": []},
                {"kind": "event", "text": "alpha unique", "mentions": [], "links": []},  # duplicate of @0
                {"kind": "event", "text": "totally separate concept", "mentions": [], "links": []},
            ],
        }).json()
...
def test_find_duplicates_returns_pairs_above_threshold(tmp_path, monkeypatch):
    with _client(tmp_path, monkeypatch, deterministic_embed) as client:
        a = client.post("/upsert-statement", json={"kind": "event", "text": "alpha unique", "mentions": [], "links": []}).json()["statement_id"]
        b = client.post("/upsert-statement", json={"kind": "event", "text": "alpha unique", "mentions": [], "links": []}).json()["statement_id"]
        c = client.post("/upsert-statement", json={"kind": "event", "text": "alpha unique", "mentions": [], "links": []}).json()["statement_id"]
        client.post("/upsert-statement", json={"kind": "event", "text": "completely separate concept", "mentions": [], "links": []})

        r = client.post("/find-duplicates", json={"threshold": 0.92, "limit": 50}).json()
        ...
        # Limit caps results.
        r_capped = client.post("/find-duplicates", json={"threshold": 0.5, "limit": 1}).json()
...
def test_delete_statement_cascades_mentions_and_links(tmp_path, monkeypatch):
    ...
    with _client(tmp_path, monkeypatch, deterministic_embed) as client:
        ...
        parent = client.post("/upsert-statement", json={"kind": "event", "text": "parent fact", "mentions": [], "links": []}).json()["statement_id"]
        child = client.post("/upsert-statement", json={"kind": "event", "text": "child fact", "mentions": [], "links": []}).json()["statement_id"]
        target = client.post("/upsert-statement", json={"kind": "event", 
            "text": "the obsolete fact about Login",
            "mentions": ["Login", "Email"],
            "links": [{"to_id": child, "link_type": "triggers"}],
            "incoming_links": [{"from_id": parent, "link_type": "contains"}],
        }).json()["statement_id"]
        x = client.post("/upsert-statement", json={"kind": "event", "text": "x", "mentions": [], "links": []}).json()["statement_id"]
        y = client.post("/upsert-statement", json={"kind": "event", "text": "y", "mentions": [], "links": []}).json()["statement_id"]
        # x --triggers (when target)--> y
        client.post("/add-links", json={"links": [
            {"from_id": x, "to_id": y, "link_type": "triggers",
        ...
        hits = client.post("/search-statements", json={"query": "obsolete fact", "limit": 10, "min_score": -1.0}).json()
        ...
        body = client.post("/get-statements", json={"ids": [parent]}).json()["statements"][0]
        assert body["links"] == []

        # The child's incoming triggers edge is gone (it came from target)
        body = client.post("/get-statements", json={"ids": [child]}).json()["statements"][0]
        assert body["incoming_links"] == []

        # And the freed vector slot is reusable — no "addPoint to update
        # deleted elements" failure on the next insert.
        r = client.post("/upsert-statement", json={"kind": "event", "text": "fresh fact", "mentions": [], "links": []})
        assert r.status_code == 200
...
def test_discover_facts_classifies_exists_near_new(tmp_path, monkeypatch):
    ...
    with _client(tmp_path, monkeypatch, shape_embed) as client:
        existing_a = client.post("/upsert-statement", json={"kind": "event", "text": "AAAA", "mentions": [], "links": []}).json()["statement_id"]
        existing_b = client.post("/upsert-statement", json={"kind": "event", "text": "BBBB", "mentions": [], "links": []}).json()["statement_id"]

        r = client.post("/discover-facts", json={
            "texts": ["AAAA", "AB", "ZZ"],
        }).json()
        assert len(r) == 3

        exact, mixed, novel = r
        assert exact == {"text": "AAAA", "status": "exists", "matches": [
            {"id": existing_a, "text": "AAAA", "score": exact["matches"][0]["score"]},
        ]}
        ...
        assert all(0.5 < m["score"] < exact["matches"][0]["score"] for m in mixed["matches"])
...
def test_discover_facts_truncates_long_text_in_matches(tmp_path, monkeypatch):
    ...
    with _client(tmp_path, monkeypatch, deterministic_embed) as client:
        client.post("/upsert-statement", json={"kind": "event", "text": long_text, "mentions": [], "links": []})
...
def test_when_expression_threads_through_link_lifecycle(tmp_path, monkeypatch):
    with _client(tmp_path, monkeypatch, deterministic_embed) as client:
        ...
        a = client.post("/upsert-statement", json={"kind": "event", "text": "validation runs", "mentions": [], "links": []}).json()["statement_id"]
        b = client.post("/upsert-statement", json={"kind": "event", "text": "invite is delivered", "mentions": [], "links": []}).json()["statement_id"]
        c = client.post("/upsert-statement", json={"kind": "event", "text": "contact info passes validation", "mentions": [], "links": []}).json()["statement_id"]
        ...
        r = client.post("/add-links", json={"links": [
            {"from_id": a, "to_id": b, "link_type": "enables",
...
def test_merge_statements_rewrites_when_references(tmp_path, monkeypatch):
    with _client(tmp_path, monkeypatch, deterministic_embed) as client:
        ...
        x = client.post("/upsert-statement", json={"kind": "event", "text": "source", "mentions": [], "links": []}).json()["statement_id"]
        y = client.post("/upsert-statement", json={"kind": "event", "text": "target", "mentions": [], "links": []}).json()["statement_id"]
        c1 = client.post("/upsert-statement", json={"kind": "event", "text": "condition one", "mentions": [], "links": []}).json()["statement_id"]
        c2 = client.post("/upsert-statement", json={"kind": "event", "text": "condition two", "mentions": [], "links": []}).json()["statement_id"]
        ...
        body = client.post("/get-statements", json={"ids": [x]}).json()["statements"][0]
        assert body["links"] == [{
            "to_id": y, "link_type": "triggers", "when": {"statement_id": c2},
        }]
...
def test_upsert_statements_batch_with_when_sibling_ref(tmp_path, monkeypatch):
    with _client(tmp_path, monkeypatch, deterministic_embed) as client:
        ...
        r = client.post("/upsert-statements", json={
            "statements": [
                {"kind": "event", "text": "validation runs", "mentions": [], "links": []},
                {"kind": "event", "text": "invite is delivered", "mentions": [], "links": []},
                {
                    "kind": "event", "text": "contact info passes validation",
                    "mentions": [], "links": [],
                    "incoming_links": [
        ...
        r2 = client.post("/upsert-statements", json={
            "statements": [
                {
                    "kind": "event", "text": "noop wrapper",
                    "mentions": [], "links": [],
                },
            ],
...
def test_replace_text_rederives_mentions_preserves_links(tmp_path, monkeypatch):
    with _client(tmp_path, monkeypatch, deterministic_embed) as client:
        client.post("/upsert-entity", json={"name": "dashboard", "description": ""})
        client.post("/upsert-entity", json={"name": "invoice", "description": ""})
        target = client.post("/upsert-statement", json={"kind": "event", "text": "target", "links": []}).json()["statement_id"]
        bid = client.post("/upsert-statement", json={"kind": "event",
            "text": "the dashboard refreshes",
            "links": [{"to_id": target, "link_type": "triggers"}],
        }).json()["statement_id"]
        body = client.post("/get-statements", json={"ids": [bid]}).json()["statements"][0]
        assert {m["name"] for m in body["mentions"]} == {"dashboard"}

        # Edit the text — mentions are RE-DERIVED from the new text (now
        # mentioning invoice, not dashboard); outgoing links are preserved.
        r = client.post("/replace-text", json={"id": bid, "text": "the invoice is sent"})
        assert r.status_code == 200
        body = client.post("/get-statements", json={"ids": [bid]}).json()["statements"][0]
        ...
        hits = client.post("/search-statements", json={"query": "the invoice is sent", "min_score": 0.99, "limit": 1}).json()
        assert hits[0]["id"] == bid

        # Unknown id → 400
        assert client.post("/replace-text", json={"id": "stm_missing", "text": "x"}).status_code == 400
...
def test_mentions_are_derived_from_text(tmp_path, monkeypatch):
    ...
    with _client(tmp_path, monkeypatch, deterministic_embed) as client:
        dashboard_id = client.post("/upsert-entity", json={"name": "dashboard", "description": ""}).json()["entity_id"]

        # Text contains the distinctive name → derives the mention.
        mentions_one = client.post("/upsert-statement", json={"kind": "event",
            "text": "the dashboard refreshes", "links": [],
        }).json()["statement_id"]
        body = client.post("/get-statements", json={"ids": [mentions_one]}).json()["statements"][0]
        assert [m["entity_id"] for m in body["mentions"]] == [dashboard_id]
        assert [m["name"] for m in body["mentions"]] == ["dashboard"]

        # Text contains no existing entity name → no mentions.
        mentions_none = client.post("/upsert-statement", json={"kind": "event",
            "text": "the system boots", "links": [],
        }).json()["statement_id"]
        body = client.post("/get-statements", json={"ids": [mentions_none]}).json()["statements"][0]
        assert body["mentions"] == []
...
def test_upsert_statements_batch_with_sibling_refs(tmp_path, monkeypatch):
    with _client(tmp_path, monkeypatch, deterministic_embed) as client:
        ...
        umbrella_id = client.post("/upsert-entity", json={"name": "umbrella", "description": ""}).json()["entity_id"]

        # An existing parent the batch will hang under via incoming_links.
        existing_parent = client.post("/upsert-statement", json={"kind": "event",
            "text": "existing parent", "links": [],
        }).json()["statement_id"]
        ...
        r = client.post("/upsert-statements", json={
            "statements": [
                ...
                {
                    "kind": "event", "text": "the umbrella covers everything",
                    "links": [
        ...
        body = client.post("/get-statements", json={"ids": [umbrella]}).json()["statements"][0]
        outgoing = sorted((l["to_id"], l["link_type"]) for l in body["links"])
        assert outgoing == sorted([(c1, "contains"), (c2, "contains"), (c3, "contains")])
        assert body["incoming_links"] == [{"from_id": existing_parent, "link_type": "contains"}]

        # Children all have an incoming contains from umbrella, no outgoing.
        for child in (c1, c2, c3):
            body = client.post("/get-statements", json={"ids": [child]}).json()["statements"][0]
            assert body["links"] == []
            assert body["incoming_links"] == [{"from_id": umbrella, "link_type": "contains"}]
    ...
    with _client(tmp_path, monkeypatch, deterministic_embed) as client:
        ...
        r = client.post("/upsert-statements", json={
            "statements": [
                ...
                {"kind": "event", "text": "second", "mentions": [], "links": [
        ...
        hits = client.post("/search-statements", json={"query": "first", "min_score": -1.0}).json()
        ...
        r = client.post("/upsert-statements", json={
            "statements": [
                {"kind": "event", "text": "alpha", "mentions": [], "links": [
...
def test_move_name_splits_into_new_entity(tmp_path, monkeypatch):
    with _client(tmp_path, monkeypatch, deterministic_embed) as client:
        ...
        hits = client.post(
            "/search-statements", json={"query": "the candidate proceeds", "min_score": 0.99}
        ).json()
        ...
        assert client.post(
            "/move-name", json={"name_id": candidate_name_id, "to_entity_id": "ent_missing"}
        ).status_code == 400
...
def test_delete_name_drops_alias_and_its_mentions(tmp_path, monkeypatch):
    ...
    with _client(tmp_path, monkeypatch, deterministic_embed) as client:
        ...
        stm_id = client.post("/upsert-statement", json={"kind": "event", 
        ...
        body = client.post("/get-statements", json={"ids": [stm_id]}).json()["statements"][0]
        ...
        # Unknown id → 400
        assert client.post(
            "/delete-name", json={"name_id": "nam_missing"}
        ).status_code == 400
...
def test_delete_entity_cascades_names_mentions_and_links(tmp_path, monkeypatch):
    ...
    with _client(tmp_path, monkeypatch, deterministic_embed) as client:
        ...
        client.post("/add-entity-links", json={"links": [
            {"from_entity_id": login_id, "to_entity_id": auth_id, "link_type": "kind-of"},
            {"from_entity_id": auth_id, "to_entity_id": login_id, "link_type": "depends-on"},
        ]})
        # Statements mentioning either alias.
        b1 = client.post("/upsert-statement", json={"kind": "event", 
            "text": "first statement touches Login", "mentions": ["Login"], "links": [],
        }).json()["statement_id"]
        b2 = client.post("/upsert-statement", json={"kind": "event", 
            "text": "second statement touches sign-in", "mentions": ["sign-in"], "links": [],
        }).json()["statement_id"]
        ...
        # Unknown id → 400
        assert client.post(
            "/delete-entity", json={"id": "ent_missing"}
        ).status_code == 400
...
def test_name_boost_lifts_statements_via_alias(tmp_path, monkeypatch):
    ...
    mention that entity."""
    with _client(tmp_path, monkeypatch, deterministic_embed) as client:
        eid = client.post("/upsert-entity", json={"name": "workstation", "description": "auth"}).json()["entity_id"]
        ...
        rel = client.post("/upsert-statement", json={
            "kind": "event",
        ...
        distractor = client.post("/upsert-statement", json={
            "kind": "event",
        ...
        hits_plain = client.post("/search-statements", json={
            "query": "dashboard",
            "limit": 5,
            "name_boost": 0.0,
        }).json()
...
def test_name_boost_inert_when_no_name_matches(tmp_path, monkeypatch):
    ...
    with _client(tmp_path, monkeypatch, deterministic_embed) as client:
        ...
        client.post("/upsert-statement", json={
            "kind": "event",
            "text": "user signs in",
            "mentions": ["Login"],
            "links": [],
        ...
        hits_on = client.post("/search-statements", json={
            "query": "qzxqzx orthogonal terms",
            "limit": 5,
        }).json()
        hits_off = client.post("/search-statements", json={
            "query": "qzxqzx orthogonal terms",
            "limit": 5,
            "name_boost": 0.0,
        }).json()
...
def test_rename_name_updates_alias_index(tmp_path, monkeypatch):
    ...
    with _client(tmp_path, monkeypatch, deterministic_embed) as client:
        eid = client.post("/upsert-entity", json={
        ...
        rename_name_id = client.post("/upsert-name", json={
            "text": "headhunter", "entity_id": eid,
        }).json()["name_id"]
        ...
        before = client.post("/search-statements", json={
            "query": "Hiring Manager", "limit": 5,
        }).json()
        before_score = next(h["score"] for h in before if h["id"] == sid)

        client.post("/rename-name", json={
            "name_id": rename_name_id, "new_text": "Hiring Manager",
        })

        # After rename: "Hiring Manager" matches the renamed name and lifts the score.
        after = client.post("/search-statements", json={
            "query": "Hiring Manager", "limit": 5,
        }).json()
...
def test_get_statement_returns_when_references(tmp_path, monkeypatch):
    ...
    with _client(tmp_path, monkeypatch, deterministic_embed) as client:
        ...
        src = client.post("/upsert-statement", json={
            "kind": "event", "text": "edge eval", "mentions": [], "links": [],
        }).json()["statement_id"]
        win = client.post("/upsert-statement", json={
            "kind": "state", "text": "win path", "mentions": [], "links": [],
        }).json()["statement_id"]
        lose = client.post("/upsert-statement", json={
            "kind": "state", "text": "lose path", "mentions": [], "links": [],
        }).json()["statement_id"]
        cond = client.post("/upsert-statement", json={
            "kind": "state", "text": "completed step is a checklist", "mentions": [], "links": [],
        }).json()["statement_id"]

        # Two edges gated on `cond`.
        r = client.post("/add-links", json={"links": [
            {"from_id": src, "to_id": win, "link_type": "establishes",
             "when": {"statement_id": cond}},
            {"from_id": src, "to_id": lose, "link_type": "establishes",
             "when": {"statement_id": cond}},
        ]})
        ...
        body = client.post("/get-statements", json={"ids": [cond]}).json()["statements"][0]
        assert body["links"] == []
        assert body["incoming_links"] == []
        ...
        body = client.post("/get-statements", json={"ids": [src]}).json()["statements"][0]
...
def test_grep_statements_alias_aware(tmp_path, monkeypatch):
    ...
    with _client(tmp_path, monkeypatch, deterministic_embed) as client:
        eid = client.post("/upsert-entity", json={
            "name": "Selection Flow", "description": "",
        }).json()["entity_id"]
        ...
        via_mention = client.post("/upsert-statement", json={
            "kind": "event",
            "text": "the selection flow is resolved for the invite",
            "links": [],
        }).json()["statement_id"]
        via_text = client.post("/upsert-statement", json={
            "kind": "event",
        ...
        r = client.post("/grep-statements", json={
            "query": "tree", "match_aliased_mentions": False,
        }).json()
        rows = {s["id"]: s for s in r["statements"]}
        assert via_text in rows
...
def test_pending_mentions_review_surface(tmp_path, monkeypatch):
    ...
    with _client(tmp_path, monkeypatch, deterministic_embed) as client:
        ...
        # Not yet a mention.
        stmt = client.post("/get-statements", json={"ids": [sid]}).json()["statements"][0]
        assert stmt["mentions"] == []

        # It is in the open review queue.
        pend = client.get("/api/pending-mentions?status=open").json()["pending_mentions"]
        assert len(pend) == 1
        ...
        assert client.get("/api/pending-mentions?status=open").json()["pending_mentions"] == []
...
def test_pending_mention_reject_writes_no_mention(tmp_path, monkeypatch):
    with _client(tmp_path, monkeypatch, deterministic_embed) as client:
        ...
        sid = client.post("/upsert-statement", json={
            "kind": "state", "text": "the data is stale", "links": [],
        }).json()["statement_id"]
        pid = client.get("/api/pending-mentions?status=open").json()["pending_mentions"][0]["id"]

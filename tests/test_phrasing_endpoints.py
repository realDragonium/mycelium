...
def test_replace_text_rejects_phrasing_violation(tmp_path, monkeypatch):
    with _client(tmp_path, monkeypatch) as client:
        ...
        bid = client.post("/upsert-statement", json={"kind": "event", 
            "text": original, "mentions": [], "links": [],
        }).json()["statement_id"]

        r = client.post("/replace-text", json={
            "id": bid,
...
def test_batch_per_item_partial_success(tmp_path, monkeypatch):
    ...
    with _client(tmp_path, monkeypatch) as client:
        r = client.post("/upsert-statements", json={
            "statements": [
                {"kind": "event", "text": "user clicks the login button", "mentions": [], "links": []},     # @0 clean
                {"kind": "event", "text": "user must verify email", "mentions": [], "links": []},           # @1 rejected
                {"kind": "event", "text": "system sends a verification email", "mentions": [], "links": []},  # @2 clean
            ],
        }).json()
...
def test_batch_per_item_bypass(tmp_path, monkeypatch):
    ...
    with _client(tmp_path, monkeypatch) as client:
        r = client.post("/upsert-statements", json={
            "statements": [
                {"kind": "event", "text": "user clicks the login button", "mentions": [], "links": []},
                {
    ...
    with _client(tmp_path, monkeypatch) as client:
        r = client.post("/upsert-statements", json={
            "statements": [
                ...
                {"kind": "event", "text": "user must verify email", "mentions": [], "links": []},
                ...
                {"kind": "event", "text": "user clicks the home button", "mentions": [], "links": []},
            ],
        }).json()
...
def test_batch_cascade_is_transitive(tmp_path, monkeypatch):
    ...
    with _client(tmp_path, monkeypatch) as client:
        r = client.post("/upsert-statements", json={
            "statements": [
                {"kind": "event", "text": "user must verify email", "mentions": [], "links": []},  # @0 rejected
                {  # @1 references @0 → cascade
                    "kind": "event", "text": "system retries the request",
                    "mentions": [], "links": [
                        {"to_id": "@0", "link_type": "triggers"},
                    ],
                },
                {  # @2 references @1 → cascade transitively
                    "kind": "event", "text": "system logs the outcome",
                    "mentions": [], "links": [
...
def test_batch_incoming_link_to_rejected_cascades(tmp_path, monkeypatch):
    ...
    with _client(tmp_path, monkeypatch) as client:
        r = client.post("/upsert-statements", json={
            "statements": [
                {"kind": "event", "text": "user must log in", "mentions": [], "links": []},  # @0 rejected
                {
                    "kind": "event", "text": "system creates the session",
                    "mentions": [], "links": [],
                    "incoming_links": [
...
def test_batch_when_ref_to_rejected_cascades(tmp_path, monkeypatch):
    ...
    with _client(tmp_path, monkeypatch) as client:
        r = client.post("/upsert-statements", json={
            "statements": [
                {"kind": "event", "text": "user must enable 2FA", "mentions": [], "links": []},  # @0 rejected
                {"kind": "event", "text": "user opens the dashboard", "mentions": [], "links": []},  # @1 clean
                {
                    "kind": "event", "text": "system sends a notification",
                    "mentions": [], "links": [
                        {"to_id": "@1", "link_type": "leads_to",
                         "when": {"statement_id": "@0"}},
                    ],
...
def test_batch_clean_items_persist_when_others_rejected(tmp_path, monkeypatch):
    ...
    with _client(tmp_path, monkeypatch) as client:
        r = client.post("/upsert-statements", json={
            "statements": [
                {"kind": "event", "text": "user must verify email", "mentions": [], "links": []},  # rejected
                {"kind": "event", "text": "user clicks login", "mentions": [], "links": []},        # clean
                {  # cascade: references rejected @0
                    "kind": "event", "text": "system sends an email",
                    "mentions": [], "links": [
                        {"to_id": "@0", "link_type": "triggers"},
                    ],
                },
                {  # bypassed
                    "kind": "event", "text": "admin can edit any post",
                    "mentions": [], "links": [],
                    "allow_phrasing_violations": True,
                },
            ],

...
- **Raw prose (`ingest_text`).** When what you hold is text rather than statements — a paragraph you were handed, a few sentences, a bullet list — pass it whole instead of pre-atomizing it. It splits on the same atomicity cues the catalog rejects compounds for (semicolons, compound phrases, conditional openers, conservative verb coordination), assigns a `kind` only where the phrasing shape is unambiguous, and runs the survivors through the same connection pipeline into the same kind of open draft, under the same catalog validation as any other write. A conditional opener also proposes the `requires` edge from claim to condition; nothing else is inferred, and nothing is rewritten — your words reach the draft or they don't reach it at all. Every fragment it can't vouch for lands in that draft as a **`flag` op**: still compound after the split, no shape matched, two shapes disagreed, or it classified and then tripped the catalog. Flags don't replay — `apply_draft` skips them — so approving the draft writes the statements and leaves the flags standing as the record of what didn't make it. Resolve one by authoring that fragment yourself (`upsert_statements`, or `submit_connected_batch` when it needs connecting) and then `discard_draft_op(draft_id, seq)` on the flag; a flag is a TODO on the draft, where `report_knowledge_gap` is a TODO on the substrate. Route by what you're holding: statements you already extracted → `submit_connected_batch`; prose the rules can segment → `ingest_text`; input too messy for either — transcripts, tickets, half-sentences — extract it yourself (or with `ingest`) and come back through `submit_connected_batch`. A draft that is mostly flags is the signal you picked the wrong door.
- **Batch new content (`submit_connected_batch`).** Extraction stays yours — the tool takes statements you have already written, runs the connection pipeline over them, and lands the batch *and* every proposal in one open draft. Define dependencies (conditions, sub-statements) at **lower** indices than the parents that reference them via `@N`, so rejections cascade upward predictably; cascade rejection is automatic, transitive, and identical to the direct write. Links are outgoing only — an item carrying `incoming_links` is rejected outright, so author an inbound edge from the other end. Two index spaces come back and they diverge the moment anything is rejected: `depends_on` reports the input positions you sent, while `batch_index` and every proposal `@N` target count accepted items only — map between them through the `batch_index` on each accepted result, never through your input order. Add `mention_hints` — name texts — to a statement whose own wording doesn't name its subject; they widen candidate discovery for that one call and never write a mention. Reach for `upsert_statements` when you want the write and nothing else: a correction, or a chunk you have already connected by hand.

### What the connection pipeline proposes — and what it won't

Three kinds of proposal, each a separate draft op carrying its provenance, none of them applied:

- **Typed links from explicit cues in your text.** Only a few pattern families cleared the precision bar and shipped: `configures` from *"can be configured on/for/via …"* on a `capability`, `composes` from arithmetic phrasing (*"equals"*, *"plus"*, *"sum of"*) on a `rule`, `restricts` from *"limits …"* and from *"is disabled/locked/frozen"* on a `state`, and `proceeds` from *"is redirected/routed/forwarded to …"* on an `event`. A cue only becomes a proposal when the two kinds admit that link type. Everything else in the text is left alone.
- **Merge candidates** — an existing statement yours duplicates, from embedding similarity plus, when the NLI extra is installed, entailment in both directions. **Mirror pairs** (`reader`/`writer`/`admin`, above/below) score ~0.99 against each other — that's expected; do **not** merge them. Merge only exact-same-claim duplicates.
- **Contradiction flags** — an existing statement yours conflicts with, filed as a knowledge gap for the operator to settle rather than resolved for you.

**Unknown connectives type themselves.** A connective the segmenter isolates between two fragments, but that no registered alias names, is embedded and compared against the whole alias set: a near-enough winner types that link *and* queues the cue as a new alias of that type — provenance `auto`, or `auto:low-confidence` when a rival type scored almost as well, each carrying the cosine that decided it. The alias add rides the draft as an `upsert_link_type_alias` op, so approving the draft is what teaches the vocabulary and rejecting it teaches nothing. A cue nothing is close to becomes a `flag` op carrying the cue and its nearest types; it never invents a link type, because a genuinely new relation meaning stays a human decision. Embeddings are direction-blind and weak on antonyms, so an automatic decision occasionally types an edge backwards — `uv run python scripts/audit_link_type_aliases.py` lists every absorption, low-confidence first, for exactly that sweep, and a curator re-asserting one with `upsert_link_type_alias` retags it `curator`. Set `MYCELIUM_CUE_RESOLUTION=strict` on the instance to skip the gate and flag every unknown cue instead.

It will **not** infer a relation that isn't in the text. Emergent behaviour, cross-feature interaction, the ordering you know from reading the code — those never had a cue to find, so they are still yours to author as `links` on the statements you submit, under §6 like any other edge. Existing statements it surfaced but wouldn't propose on come back under `related`; that list is where the link you should author yourself usually is.

Nothing lands until you say so: read the draft with `get_draft`, strike anything you disagree with via `discard_draft_op(draft_id, seq)`, then `submit_draft(draft_id)`. A statement the pipeline finds nothing for is a valid outcome, not a failure — but it doesn't relax §6: a statement that ends up with no link in or out is still yours to wire.
